...
img_height=256
img_width=256
interpolation='bicubic' #https://en.wikipedia.org/wiki/Image_scaling
kernel_size = (3,3)
labels='inferred'
label_mode='categorical'
model_name='flower_model_v_2.h5'
...
val_set = make_dataset('validation') # Rescaling(1./255)
...
model = Sequential([
    ...
    # 1st convolution
    Conv2D(no_of_filters, kernel_size, strides=strides, activation='relu'),
    MaxPool2D(),
    # 2nd convolution
    Conv2D(no_of_filters, kernel_size, strides=strides, activation='relu'),
    MaxPool2D(),
...
model.compile(
    ...
    metrics=[Precision(name='precision'), 'categorical_accuracy'] 
)
...
validation_result = model.evaluate(val_set, return_dict=True)

from google.colab import drive
from json import dumps

drive.mount('/content/gdrive')
model.save(f'{model_save_path}/{model_name}')

evaluation_filename = f'{model_save_path}/{model_name}_eval.txt'
with (open(evaluation_filename,'w')) as f:
 
  f.write(dumps(validation_result))
...
chart_name = f'{model_name}_chart.png'
chart_file = f'{model_save_path}/{chart_name}'
